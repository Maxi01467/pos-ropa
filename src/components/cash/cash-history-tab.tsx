...
export function CashHistoryTab() {
    ...
    return (
        <div className="animate-in fade-in duration-300">
            ...
            >
                <DialogContent className="max-h-[90vh] grid-rows-[auto_auto_auto_minmax(0,1fr)] overflow-hidden sm:max-w-5xl">
                    {selectedTicketsSession && (
                        <>
                            <DialogHeader>
                                <DialogTitle className="flex items-center gap-2 text-xl">
                            ...
                            <div
                                className="min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain rounded-[1.25rem] border border-border/70"
            ...
            >
                <DialogContent className="max-h-[90vh] grid-rows-[auto_auto_minmax(0,1fr)] overflow-hidden sm:max-w-3xl">
                    {selectedTicketDetail && (
                        <>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    {formatDate(selectedTicketDetail.createdAt)} · {selectedTicketDetail.sellerName}
                                </DialogDescription>
                            </DialogHeader>
                            ...
                            <div className="grid gap-3 sm:grid-cols-3">
                                <div
                                    className={cn(
                                        ...
                                        selectedTicketDetail.paymentMethod === "EFECTIVO" && "border-emerald-900/30 bg-emerald-50 text-emerald-950 dark:bg-emerald-950/30 dark:text-emerald-100",
                                        selectedTicketDetail.paymentMethod === "TRANSFERENCIA" && "border-blue-900/30 bg-blue-50 text-blue-950 dark:bg-blue-950/30 dark:text-blue-100",
                                        selectedTicketDetail.paymentMethod === "MIXTO" && "border-violet-900/30 bg-violet-50 text-violet-950 dark:bg-violet-950/30 dark:text-violet-100",
                                        selectedTicketDetail.paymentMethod === "CAMBIO" && "border-orange-900/30 bg-orange-50 text-orange-950 dark:bg-orange-950/30 dark:text-orange-100"
                                    )}
                                >
                                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Método</p>
                                    <p className="mt-2 text-sm font-bold">{selectedTicketDetail.paymentMethod}</p>
                                </div>
                                <div className="rounded-[1.25rem] border border-border/70 bg-muted/30 p-4">
                                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Productos</p>
                                    <p className="mt-2 text-sm font-bold">{selectedTicketDetail.items.length}</p>
                                </div>
                                <div className="rounded-[1.25rem] border border-border/70 bg-muted/30 p-4">
                                    <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total</p>
                                    <p className="mt-2 text-sm font-bold text-emerald-700 dark:text-emerald-300">{formatCurrency(selectedTicketDetail.total)}</p>
                                </div>
                            </div>
                                        ...
                                        {selectedTicketDetail.items.length === 0 ? (
                                            ...
                                            selectedTicketDetail.items.map((item) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="text-right">{formatCurrency(item.priceAtTime)}</TableCell>
